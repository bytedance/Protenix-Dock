...
class ReceptorGeometry(BaseGeometry):
    ...
    def torsion_to_xyz(self, torsion: np.ndarray, xyz0: np.ndarray):
        ...
        for frag_size, data in self.data.items():
            ...
            u = u / np.clip((u**2).sum(axis=-1, keepdims=True), 1e-5, None)**0.5
            ...
            out = (
                np.einsum(
                    "...ij,...kj->...ki",
                    R,
                    (xyz_frag - np.expand_dims(rot_bond_xyz1, axis=-2)),
                )
                + np.expand_dims(rot_bond_xyz1, axis=-2)
            )  # [..., num_fragments, num_atoms_in_fragment, 3]
            out = out.reshape(
...
class LigandGeometry(BaseGeometry):
    def __init__(
        ...
        # O(1) lookup dicts for torsion6_to_xyz inner loop
        self._permute_index_lookup = {atom: idx for idx, atom in enumerate(self.permute_index)}
        self._fragment_index_lookup = {}
        for frag_id, frag in enumerate(self.fragments):
            for local_idx, atom in enumerate(frag):
                self._fragment_index_lookup[(frag_id, atom)] = local_idx
    ...
    def permute_xyz(self, xyz: np.ndarray, reverse: bool = False):
        ...
        if reverse:
            permute_index = self.reverse_index
        else:
    ...
    def torsion6_to_xyz(
        ...
        if len(translaton.shape) < len(xyz.shape):
            translaton = np.expand_dims(translaton, axis=-2)
            ...
            for i, frag_data in level.items():
                ...
                if len(parent) > 0:
                    ...
                    atom_xyz0, parent_xyz0 = (
                        xyz[..., self._permute_index_lookup[atom], :],
                        xyz[..., self._permute_index_lookup[parent_atom], :],
                    )
                    ...
                    xyz_fragments[i] = np.einsum(
                        "...ij,...kj->...ki",
                        R,
                        xyz_fragments[i] - np.expand_dims(parent_xyz0, axis=-2),
                    ) + np.expand_dims(parent_xyz[i], axis=-2)
                ...
                for child in children_list:
                    ...
                    parent_xyz[child] = xyz_fragments[i][
                        ..., self._fragment_index_lookup[(i, atom)], :
                    ]
